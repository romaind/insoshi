module Paperclip
  ...
  class Attachment
    ...
    def reprocess_for_video_thumnbails!
      new_original = Tempfile.new("paperclip-reprocess")
      if old_original = to_file(:original)
        new_original.write( old_original.read )
        new_original.rewind
    ...
    def post_process_video #:nodoc:
      return if @queued_for_write[:original].nil?
      logger.info("[paperclip] Post-processing video #{name}")
      file_for_thumbnail = Tempfile.new("video_thumbnail")

      command = <<-end_command
      ffmpeg -i #{ @queued_for_write[:original].path } -an -ss 00:00:01 -f image2 -r 1 -vframes 1 -y #{ file_for_thumbnail.path }
      end_command
