...
describe ActivitiesHelper do
  ...
  it "should have the right message for a wall comment" do
    ...
    login_as(:kelly)
    feed_message(activity).should =~ /#{commenter.last_name}/
    feed_message(activity).should =~ /#{person.last_name}'s wall/
  ...
  it "should have the right message for an own-comment" do
    ...
    login_as(:kelly)
    feed_message(activity).should =~ /#{commenter.last_name}/
    feed_message(activity).should =~ /#{commenter.last_name}'s wall/
  end
